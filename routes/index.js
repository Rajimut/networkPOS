...
module.exports = function(router, passport) {
    ...
    router.post('/buyer-signup',
        ...
        function(req, res) {
            ...
            User.update({
                ...
                $set: {
                    'local.customer_flag': req.body.customertype
                }
            }, function(err) {
    ...
    router.get('/myreceipts', isLoggedIn, function(req, res) {
        ...
        InvoiceDetail.find({
                'buyer_name': req.user.local.email
            }).populate('seller_id', 'seller_logo seller_name', SellerDB)
            .exec(function(err, invoice) {
                if (err) {
                    res.send("There was an error looking up records for buyer " + req.user.local.email + ":" + err);
                } else {
                    res.render('myreceipts', {
                        myreceipt_: invoice
                    });
                }
            });
        ...
        InvoiceDetail.find({
            ...
            .exec(function(err, invoice) {
                ...
                if (err) {
                    ...
                    res.render('mixins/mixin-receipt.jade', {
                        receipt: invoice[0]
                    });
    ...
    /* Angular JS - Added by Raji */
    router.get('/api/components', isLoggedIn, function(req, res) {
        InvoiceDetail.find({
            'buyer_name': req.user.local.email
        }, function(err, invoice) {
            if (err) {
